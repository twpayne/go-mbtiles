...
func TestReaderSelectTile(t *testing.T) {
	mbtrCache := make(map[string]*Reader)
	for _, tc := range []struct {
		...
		if !ok {
			var err error
			mbtr, err = NewReader(tc.dsn)
			if err != nil {
				t.Errorf("NewReader(%q) == %v, %v, want _, <nil>", tc.dsn, mbtr, err)
				continue
			}
		...
		if err != nil {
			t.Errorf("NewReader(%q).SelectTile(%d, %d, %d) == %v, %v, want _, <nil>", tc.dsn, tc.z, tc.x, tc.y)
			continue
		}
		if sha256sum := sha256.Sum256(tileData); sha256sum != tc.sha256sum {
			t.Errorf("NewReader(%q).SelectTile(%d, %d, %d) tile data has SHA256 sum %s, want %s", tc.dsn, tc.z, tc.x, tc.y, hex.EncodeToString(sha256sum[:]), hex.EncodeToString(tc.sha256sum[:]))
		}
	}
	for dsn, mbtr := range mbtrCache {
		if err := mbtr.Close(); err != nil {
			t.Errorf("NewReader(%q).Close() == %v, want <nil>", dsn, err)
		}
	}
}
